...
subroutine get_chern_number_from_wcc(Noccpied, Numk, wcc, IChern_number)
   ...
   integer, intent(out) :: IChern_number
   ...
   IChern_number = ANINT(Chern_number) 
   !IChern_number = INT(Chern_number)
...
subroutine  Z2_3D
   ...
   if (cpuid==0) then
      ...
      close(outfileindex)
      call execute_command_line ('gnuplot -persist wanniercenter3D_Z2.gnu')! Now plot the results
